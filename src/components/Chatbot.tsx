        ...
        <div className="relative">
          ...
          <div className="mb-4 rounded-2xl shadow-xl border border-orange-200 p-4 w-64 bg-[ffea00] bg-[#ffea00]">
            ...
            <div className="mt-4 flex space-x-2">
              ...
              <Button onClick={() => setIsOpen(true)} className="flex-1 border border-gray-300 text-xs py-2 px-3 rounded-lg flex items-center justify-center space-x-1 text-slate-50 bg-green-500 hover:bg-green-400">
      ...
      <Card className="w-80 h-[500px] bg-white shadow-2xl border border-orange-200 rounded-2xl">
        <CardHeader className="bg-gradient-to-r from-orange-400 to-orange-500 p-4 rounded-t-2xl">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <div className="relative">
                <img src="/lovable-uploads/035b95d1-aec7-425d-841a-b471202969a3.png" alt="Support Agent" className="w-10 h-10 rounded-full object-cover" />
        ...
        <CardContent className="p-0 flex flex-col h-[calc(500px-80px)]">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.length === 0 && <div className="text-center text-gray-500 text-sm">
                <p>👋 Hi! I'm Quickbot.</p>
            ...
            {messages.map(message => <div key={message.id} className={`flex ${message.isUser ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] p-3 rounded-lg text-sm ${message.isUser ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-800'}`}>
                  {message.text}
                </div>
              </div>)}
          ...
          <div className="p-4 border-t border-gray-200">
            <div className="flex space-x-2">
              <Input value={inputValue} onChange={e => setInputValue(e.target.value)} onKeyPress={handleKeyPress} placeholder="Type your message..." className="flex-1" />
              <Button onClick={handleSendMessage} className="bg-orange-500 hover:bg-orange-600 text-white px-3" disabled={!inputValue.trim() || isTyping}>
